...
def train(
    env_fn, g=0.99, steps: int = 10_000, seed: int | None = 0, **env_kwargs
):
    # Train a single model to play as each agent in a rival parallel environment
    ...
    model.save(f"{env.unwrapped.metadata.get('name')}_gamma{g}_{time.strftime('%Y%m%d-%H%M%S')}")
...
if __name__ == "__main__":
    env_fn = ecosystem_parallel
    env_kwargs = {"render_mode": "None"}
    training_gamma = 0.95

    # Train a model
    train(env_fn, training_gamma, steps=20_000_000, seed=0, **env_kwargs)
